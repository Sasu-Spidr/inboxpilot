...
class Connector:
    ...
    def unread_emails(self, limit): return [{"id":"1","subject":"Need help","sender":"a@b.com","body":"Hello","thread_id":"t"}]
    def get_email(self, message_id): return {"id":message_id,"subject":"Need help","sender":"a@b.com","body":"Hello","thread_id":"t"}
    def apply_label(self,*args): self.calls.append(("label",args))
    def create_draft(self,*args): self.calls.append(("draft",args))
    def trash(self,*args): self.calls.append(("trash",args))
    def mark_read(self,*args): self.calls.append(("read",args))
...
class State:
    def __init__(self): self.records=set(); self.completed=[]
    def is_processed(self,*args): return args in self.records
    def begin(self, **kwargs): self.records.add((kwargs["client_id"], kwargs["connector"], kwargs["account"], kwargs["message_id"]))
    def complete(self, **kwargs): self.completed.append(kwargs)
    def remove(self,*args): self.records.discard(args)

def test_worker_creates_draft(monkeypatch):
    monkeypatch.chdir(Path(__file__).parents[1])
    c=Connector(); settings={"groq_api_key":"x","max_emails_per_cycle":1,"token_encryption_key":"x"}
    ...
    worker.run_cycle()
    assert [x[0] for x in c.calls] == ["label", "draft", "read"]

def test_worker_does_not_process_duplicate(monkeypatch):
    monkeypatch.chdir(Path(__file__).parents[1])
    c=Connector(); state=State(); settings={"groq_api_key":"x","max_emails_per_cycle":1,"token_encryption_key":"x"}
    worker=MailWorker(settings, connectors={"exuvie":{"gmail:main":{"name":"gmail","account":"main","connector":c}}}, classifier=Classifier(), drafts=Drafts(), state=state)
    worker.run_cycle()
    worker.run_cycle()
...
def test_error_on_one_email_does_not_block_next(monkeypatch):
    ...
    class MultiConnector(Connector):
        def unread_emails(self, limit):
            return [{"id":"bad","subject":"Need help","sender":"a@b.com","body":"Hello","thread_id":"t"}, {"id":"ok","subject":"Need help","sender":"a@b.com","body":"Hello","thread_id":"t"}]
        def create_draft(self, email, text):
            if email["id"] == "bad":
                raise RuntimeError("draft failed")
            super().create_draft(email, text)
    c=MultiConnector(); settings={"groq_api_key":"x","max_emails_per_cycle":2,"token_encryption_key":"x"}
    worker=MailWorker(settings, connectors={"exuvie":{"gmail:main":{"name":"gmail","account":"main","connector":c}}}, classifier=Classifier(), drafts=Drafts(), state=State())
    worker.run_cycle()
...
def test_multi_client_multi_mailbox(monkeypatch):
    monkeypatch.chdir(Path(__file__).parents[1])
    g=Connector(); h=Connector(); settings={"groq_api_key":"x","max_emails_per_cycle":1,"token_encryption_key":"x"}
    connectors={
        "exuvie":{
