...
def test_replace_label_stays_message_scoped_even_when_thread_exists():
    calls = []

    class LabelOps:
        def list(self, **kwargs):
            return Execute({"labels": [{"name": "Commercial", "id": "commercial"}, {"name": "Marketing", "id": "legacy-marketing"}]})

    class MessageOps:
        def get(self, **kwargs):
            return CaptureExecute({"threadId": "thread-1"}, calls, "get_message", kwargs)
    ...
    class ThreadOps:
        def modify(self, **kwargs):
            return CaptureExecute({}, calls, "modify_thread", kwargs)
...
def test_replace_label_removes_gmail_promotions_system_category():
    calls = []
    ...
    class LabelOps:
        def list(self, **kwargs):
            return Execute(
                {
                    "labels": [
                        {"name": "Commercial", "id": "commercial", "type": "user"},
                        {"name": "CATEGORY_PROMOTIONS", "id": "CATEGORY_PROMOTIONS", "type": "system"},
                    ]
                }
            )
    ...
    assert calls == [
        (
            "modify_message",
            {
                "userId": "me",
                "id": "msg-1",
                "body": {"addLabelIds": ["commercial"], "removeLabelIds": ["CATEGORY_PROMOTIONS"]},
            },
        )
    ]
